"""s06: pause tool execution for an explicit approval decision.

The model is scripted so the example runs offline. Reads run automatically,
while a structured file patch is proposed, approved, and only then executed.
"""
...
class ApprovalOrchestrator:
    """Classify, pause for a decision when needed, then execute or reject."""
...
class Workspace:
    ...
    def preview_patch(self, patch: str) -> PatchPlan:
        """Parse and verify a patch without committing filesystem changes."""
        plan = parse_patch(patch)
        self._verify(plan)
        return plan
