"""s07: enforce runtime permissions after an explicit approval decision.

The model is scripted so the example runs offline. A structured file patch is
approved, then either committed or denied by the active workspace sandbox.
"""
...
def main() -> None:
    ...
    with TemporaryDirectory() as directory:
        ...
        profile = PermissionProfile.read_only(root)
        router = default_router(
            Workspace(root, profile),
            decider=approve_for_session,
            event_sink=consume,
        )
        print(f"active permission profile: {profile.name}")
        print(
            "model-visible tools:",
