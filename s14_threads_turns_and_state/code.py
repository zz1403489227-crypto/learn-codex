"""s14: keep thread identity, turn execution, and runtime state separate.

The inherited runtime remains offline. This chapter keeps the previous
context/skills/plans runtime and wraps it in a small thread manager:
thread metadata is durable, each turn gets its own execution record, and
pending approvals/input/cancellation live only in the active turn state.
"""
